...
public class Camera2BasicFragment extends Fragment
    ...
    /**
     * If argument passed to activity is nonzero, generate at least this much data and exit:
     */
    private long generateLength;

    /**
     * This is a (thread-safe) "dictionary" for storing our pictures.
     */
    private long[] picMillis;
    private byte[][] picBytes;
    private int[] rowStrides;
    private AtomicInteger picIndex;
    private AtomicInteger picFinishedIndex;

    /**
     * Don't interrupt while taking a picture.
     */
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            picMillis[index] = System.currentTimeMillis();
            mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), index, picBytes, rowStrides, new  Callback() {
                @Override
                public void callback() {
                    ...
                    if (finishedCount >= ((1<<timeCorrectionOrder) * timeMultiple)) {
                        ...
                        BitSet outBools = getNoiseBools(sortedPicBytes);

                        // TODO: Consider writing to disk instead:
                        RandSingleton.getInstance().randBools = outBools;
                        RandSingleton.getInstance().randSize = outBools.size();
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        picIndex = new AtomicInteger();
        picFinishedIndex = new AtomicInteger();
        isTakingPicture = new AtomicInteger();
        isTakingPicture.set(0);
        isResettingCamera = new AtomicInteger();
        isResettingCamera.set(0);

        CameraActivity cameraActivity = (CameraActivity)getActivity();
        generateLength = cameraActivity.generateLength;
    ...
    private void ControlCallback() {
        ...
        showToast("Success! (Control case)");
    ...
    private void ResetCamera() {
        int irc = isResettingCamera.getAndIncrement();
        if (irc == 0) {
            closeCamera();
            stopBackgroundThread();
            //unlockFocus();
            ...
            try {
                ...
                Log.d(TAG, "Tried to reset camera while activity is closing.");
                e.printStackTrace();
                return;
            }
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                //Integer bayer = characteristics.get(CameraCharacteristics.SENSOR_INFO_COLOR_FILTER_ARRANGEMENT);
    ...
    private void turnOffNoiseFilters(CaptureRequest.Builder captureBuilder) {
    ...
    private void captureStillPicture() {
        try {
            ...
            // We're going to create a new repeating request, and we can only have one at a time:
            //stopBackgroundThread();
            ...
            captureBuilder.set(CaptureRequest.CONTROL_AF_MODE,
                    CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE);
            turnOffNoiseFilters(captureBuilder);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.picture: {
                int itp = isTakingPicture.getAndIncrement();
                if (itp == 0) {
    ...
    /**
     * Callback when ImageSaver is done.
     */
    interface Callback {
    ...
    private static class ImageSaver implements Runnable {
        ...
        /**
         * Output for image bytes.
         */
        private final byte[][] mPicBytes;
        private int[] mPicRowStride;
        ...
        ImageSaver(Image image, int index, byte[][] picBytes, int[] picRowStride, Callback c) {
            mImage = image;
            mIndex = index;
            mPicBytes = picBytes;
            mPicRowStride = picRowStride;
            mCallback = c;
        }
        ...
        public void run() {
            try {
                mPicRowStride[mIndex] = mImage.getPlanes()[0].getRowStride();
                //int pixelStride = mImage.getPlanes()[0].getPixelStride();
                ByteBuffer buffer = mImage.getPlanes()[0].getBuffer();
                byte[] bytes = new byte[buffer.remaining()];
                buffer.get(bytes);
                mImage.close();
                mPicBytes[mIndex] = bytes;

                Runtime.getRuntime().gc();

                mCallback.callback();
            } catch (IllegalStateException e) {
                Log.d(TAG, "Trying to process closed image.");
            }
        }
    }
    ...
    public BitSet getNoiseBools(byte[][] frameBytes) {
        short[][] interIn = new short[frameBytes.length][];

        // Convert to 16 bit Bayer mosaic array
        for (int i = 0; i < frameBytes.length; i++) {
            ByteBuffer bc = ByteBuffer.wrap(frameBytes[i]);
            bc.rewind();
            interIn[i] = new short[frameBytes[i].length / 2];
            bc.asShortBuffer().get(interIn[i]);
        }

        frameBytes = null;
        Runtime.getRuntime().gc();

        // We'll use just the green channels:
        int height, width;
        width = mWidth / 2;
        height = mHeight;
        int bytesPerPixel = 1;
        ...
        for (int i = 0; i < interIn.length; i++) {
            short[] interOut = new short[interIn[i].length / 2];
            for (int j = 0; j < height; j+=2) {
                for (int k = 0; k < width; k++) {
                    interOut[(j * width) + k] = interIn[i][(j * mWidth) + (k * 2 + 1)];
                }
            }
            for (int j = 1; j < height; j+=2) {
                for (int k = 0; k < width; k++) {
                    interOut[(j * width) + k] = interIn[i][(j * mWidth) + (k * 2)];
                }
            }
            interIn[i] = interOut;
        }

        int blockHeight = 1 << spaceCorrectionOrder;
        int blockWidth = 1 << (spaceCorrectionOrder - 1);
        int blockSize = blockHeight * blockWidth;
        int heightBlocks = height / blockHeight;
        int widthBlocks = width / blockWidth;
        sz = heightBlocks * widthBlocks * blockSize;

        // Mean-centering in each block:
        int hb, wb, h, w, px;
        int bSum;
        ...
        for (int frame = 0; frame < interIn.length; frame++) {
            ...
            for (hb = 0; hb < heightBlocks; hb++) {
                ...
                for (wb = 0; wb < widthBlocks; wb++) {
                    bSum = 0;
                    wbBlockWidth = wb * blockWidth;
                    for (h = 0; h < blockHeight; h++) {
                        for (w = 0; w < blockWidth; w++) {
                            bSum += interIn[frame][(wbBlockWidth + w) + (width * (hbBlockHeight + h))];
                        }
                    }

                    mn = bSum / (double)blockSize;

                    for (h = 0; h < blockHeight; h++) {
                        for (w = 0; w < blockWidth; w++) {
                            px = (wbBlockWidth + w) + (width * (hbBlockHeight + h));
                            interOut[px] += interIn[frame][px] - mn;
                        }
                    }
                }
            }
            interIn[frame] = interOut;
        }

        width = widthBlocks * blockWidth;
        height = heightBlocks * blockHeight;

        int time, dimen, group;
        int tempWidth, tempHeight;
        int lineTempWidthBpp;
        int bytesPerPixelX2 = bytesPerPixel * 2;
        int tempI;
        for (int deriv = 1; deriv <= spaceCorrectionOrder; deriv++) {
            // Space derivatives:
            for (time = 0; time < interIn.length; time++) {
                short[] interSpaceIn;
                short[] interSpaceOut;

                tempHeight = height >> deriv;
                tempWidth = width >> (deriv - 1);
                sz = maxSz >> (2 * deriv - 1);
                interSpaceIn = interIn[time];
                interSpaceOut = new short[sz * bytesPerPixel];
                for (dimen = 0; dimen < bytesPerPixel; dimen++) {
                    for (int line = 0; line < tempHeight; line++) {
                        lineTempWidthBpp = line * tempWidth * bytesPerPixel;
                        for (group = 0; group < tempWidth; group++) {
                            tempI = lineTempWidthBpp + (group * bytesPerPixel) + dimen;
                            interSpaceOut[tempI] =
                                    (short) (interSpaceIn[tempI + lineTempWidthBpp]
                                            ^ interSpaceIn[tempI + lineTempWidthBpp + (tempWidth * bytesPerPixel)]);
                        }
                    }
                }
                interIn[time] = interSpaceOut;

                // For Bayer mosaic, one more derivative is needed along height dimension, (in this array-packing convention,) to "square" the data.
                if (deriv < spaceCorrectionOrder) {
                    sz = maxSz >> (2 * deriv);
                    interSpaceOut = new short[sz * bytesPerPixel];
                    int groupCount = sz;
                    for (dimen = 0; dimen < bytesPerPixel; dimen++) {
                        for (group = 0; group < groupCount; group++) {
                            tempI = (group * bytesPerPixel) + dimen;
                            interSpaceOut[tempI] =
                                    (short) (interSpaceIn[tempI + bytesPerPixel]
                                            ^ interSpaceIn[tempI + bytesPerPixel * 2]);
                        }
                    }
                    interIn[time] = interSpaceOut;
                }
        ...
        for (time = 0; time < interIn.length; time++) {
            for (int i = 0; i < sz; i++) {
                for (int deriv = 1; deriv <= 4; deriv++) {
                    int groupCount = 16 >> deriv;

                    int b1 = interIn[time][i] & mask1;
                    int b2 = (interIn[time][i] & mask2) >> 1;
                    int bo = b1 ^ b2;
        ...
        for (time = 0; time < interIn.length; time++) {
            for (int i = 0; i < sz; i++) {
                if (interIn[time][i] > 0) {
                    bitsOut.set(i + (time * sz));
                    oneCount++;
                } else {
                    bitsOut.clear(i + (time * sz));
                    zeroCount++;
                }
            }
        }

        //showToast("1: " + Float.toString(((float)oneCount)/sz) + "0: " + Float.toString(((float)zeroCount)/sz));
