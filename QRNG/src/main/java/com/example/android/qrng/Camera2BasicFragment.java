...
public class Camera2BasicFragment extends Fragment
    ...
    public BitSet getNoiseBools(byte[][] frameBytes) {
            ...
            for (int j = 0; j < height; j++) {
                if ((j % 2) == 0) {
                    for (int k = 0; k < mWidth / 2; k++) {
                        interOut[(j * mWidth / 2) + k] = interIn[i][(j * mWidth) + (k * 2 + 1)];
                    }
                } else {
                    for (int k = 0; k < mWidth / 2; k++) {
                        interOut[(j * mWidth / 2) + k] = interIn[i][(j * mWidth) + (k * 2)];
                    }
                }
            }
            interIn[i] = interOut;
                ...
                for (wb = 0; wb < widthBlocks; wb++) {
                    ...
                    for (h = 0; h < blockHeight; h++) {
                        for (w = 0; w < blockWidth; w++) {
                            px = (wb * blockWidth + w) + (width * (hb * blockHeight + h));
                            bSum += interIn[frame][px];
                        }
                    }
                ...
                for (dimen = 0; dimen < bytesPerPixel; dimen++) {
                    for (int line = 0; line < tempHeight; line++) {
                        for (group = 0; group < tempWidth; group++) {
                            interSpaceOut[(line * tempWidth * bytesPerPixel) + (group * bytesPerPixel) + dimen] =
                                    (short) (interSpaceIn[(line * tempWidth * 2 * bytesPerPixel) + (group * bytesPerPixel) + dimen]
                                            ^ interSpaceIn[(line * tempWidth * 2 * bytesPerPixel) + (group * bytesPerPixel) + (tempWidth * bytesPerPixel) + dimen]);
                        }
                    }
                }
                ...
                if (deriv < spaceCorrectionOrder) {
                    ...
                    for (dimen = 0; dimen < bytesPerPixel; dimen++) {
                        for (group = 0; group < groupCount; group++) {
                            interSpaceOut[(group * bytesPerPixel) + dimen] =
                                    (short) (interSpaceIn[(group * 2 * bytesPerPixel) + dimen]
                                            ^ interSpaceIn[(group * 2 * bytesPerPixel) + dimen + bytesPerPixel]);
                        }
                    }
                    interIn[time] = interSpaceOut;
                }
        ...
        //showToast("1: " + Float.toString(((float)oneCount)/(bitsOut.length)) + "0: " + Float.toString(((float)zeroCount)/(bitsOut.length)));
